...
(c) install Privacy Badger, Ublock Origin, and NoScript;

(d) enable custom .css in Firefox by setting "toolkit.legacyUserProfileCustomizations.stylesheets" to "true" in about:config;

(e) keep "browser.bookmarks.editDialog.showForNewBookmarks" set to "true" in about:config; the checkbox for this setting and the associated text has been removed from the actual "Add Bookmark" popup.

(f) in Firefox's about:config, set "full-screen-api.enabled" to "false" (this prevents tapping to fullscreen on a YouTube video while preserving Mod+F for an enlarged player);
...
(n) be already subscribed to the YouTube channels you want to be subscribed to; the current YouTube userContent.css file hides the subscribe button on YouTube profiles, and this is listed in problems-to-be-fixed.txt. It will be fixed soon, but it is not an urgent priority for me;

(o) in Thunar, press Ctrl+M, click "View," and uncheck "Show File Highlight";
